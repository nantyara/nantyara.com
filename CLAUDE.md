...
- **養子縁組**: `timetree_id` を持たない同日・同会場（正規化比較）のエントリが既にある場合
  （sync-x や手動で先に追加されたケース）、新規作成せずそのエントリに `timetree_id` を
  書き足して紐づける。候補が複数あるときは曖昧なのでスキップして警告（手動解決）
...
- 必須フィールドの欠落
- ID/Slugの重複
- **同日・同会場の重複**（会場名は空白除去+小文字化で正規化して比較。取り込み経路が
  複数ある[TimeTree/X/手動]ため、同一イベントの二重登録をコミット時に堰き止める。
  昼夜2公演など正当な重複は `duplicate_ok: true` で明示的に通す）
- 日付フォーマット
